...
@WebServlet("/UpdateBookReg")
public class UpdateBookRegCtrl extends HttpServlet {
	private static final long serialVersionUID = 1L;


	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Update Book Registration  doGet Starts");
		...
		} else if (button.equalsIgnoreCase("UpdateBook")) {
		...
		boolean bkstatus = LibraryInfoDao.updateBook(bookRegDto);
			if(bkstatus) {
				out.println("<h1 style='color:green;margin-top:100px;'>"+bookId+" , was Updated successfully</h1>");
				out.println("<h1 style='color:black;margin-top:120px;'><a href='index.jsp'>Home</a></h1>");

				rd = request.getRequestDispatcher("index.jsp");
				rd.include(request, response);
				} else {
				out.println("<h1 style='color:red;margin-top:100px;'>Error in Update book</h1>");
				rd = request.getRequestDispatcher("UpdateBook.jsp");
				rd.include(request, response);
				System.out.println("Error in Update book");
			}
		}//rd.forward(request, response);
		System.out.println("Book Registration Update doGet ends");
